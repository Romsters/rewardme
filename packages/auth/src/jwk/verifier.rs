...
impl JwkVerifier {
    pub fn new(keys: Vec<JwkKey>, configuration: JwkConfiguration) -> JwkVerifier {
        JwkVerifier {
    ...
    fn decode_token_with_key(
        ...
        validation.iss = Some(vec![self.config.issuer.clone()].into_iter().collect());
        let key = DecodingKey::from_rsa_components(&key.n, &key.e);
        return decode::<Claims>(token, &key.unwrap(), &validation)
